...
if request("tj")="Add" or request("tj")="Modify" then
    ...
    if request("tj")="Add" then
		sql="select count(id) from menu1 where name1='"&request.Form("name1")&"' "
	else
		sql="select count(id) from menu1 where name1='"&request.Form("name1")&"' and id<>"&request("id")&" "
	end if
...
if request("tj")="Add" then
	...
	rsrz("caozuo")="添加"
	rsrz("menu")=menuname
	rsrz("duixian")="一级栏目："&Request("name1")
	rsrz.update
	rsrz.close
...
if request("tj")="Modify" then
	sql="select * from menu1 where id="&request("id")
...
<strong>添加一级<%=menunamel%></strong><br />
...
<table  border="1" align="center" cellpadding="3" cellspacing="0" bordercolorlight="#C0C0C0" bordercolordark="#FFFFFF">
	...
	<tr>
	  <td align="center" bgcolor="#F0F0F0"><strong>显示位置：</strong></td>
	  <td align="left">
		<%for z=0 to ubound(weizhi)%>
		 <label style="white-space:nowrap;<%if z=0 then%>color:#FF0000;<%end if%>">
		 <input type="checkbox" class="dq" name="weizhi" id="weizhi" value="<%=weizhi(z)%>" onpropertychange="red(this)" <%if z=0 then%>checked="checked"<%end if%> />
		 <%=weizhi(z)%></label>
		<%next%>
		<span class="red">*</span>
		</td>
...
%>
<strong>修改一级<%=menunamel%></strong><br />
<span class="red"><%=chicun_sm%></span><br><br>
...
<table border="1" align="center" cellpadding="3" cellspacing="0" bordercolorlight="#C0C0C0" bordercolordark="#FFFFFF">
	...
	<tr>
	  ...
	  <td align="left">
		<%for z=0 to ubound(weizhi)%>
		 <label style="white-space:nowrap;<%if instr(""&rs("weizhi"),weizhi(z)) then%>color:#FF0000;<%end if%>">
		 <input type="checkbox" class="dq" name="weizhi" id="weizhi" value="<%=weizhi(z)%>" onpropertychange="red(this)" <%if instr(""&rs("weizhi"),weizhi(z)) then%>checked="checked"<%end if%> />
		 <%=weizhi(z)%></label>
		<%next%>
		<span class="red">*</span>		</td>
	</tr>
