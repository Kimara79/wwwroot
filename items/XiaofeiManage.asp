...
if request("cz")="del" then
	sql="delete * from xiaofei where id="&request("id")               '删除数据
...
if request("xs")="all" then
	session("xiaofei_all")="1"
	session("xiaofei_search_type")=""
	session("xiaofei_search_key")=""
end if

if request("search_key")<>"" then
	session("xiaofei_all")="0"
	session("xiaofei_search_type")=request("search_type")
	session("xiaofei_search_key")=request("search_key")
end if

if request("page")<>"" then
	session("xiaofei_page")=request("page")
end if
if session("xiaofei_page")="" then
	session("xiaofei_page")=1
end if

sqladd=""
if session("xiaofei_search_key")<>"" then
	select case session("xiaofei_search_type")
		case "users_card"
			sqladd=sqladd&" and usersid in (select id from users where card="&session("xiaofei_search_key")&" ) "
		case "users_name1"
			sqladd=sqladd&" and usersid in (select id from users where name1 like '%"&session("xiaofei_search_key")&"%' ) "
		case else
			sqladd=sqladd&" and "&session("xiaofei_search_type")&" like '%"&session("xiaofei_search_key")&"%' "
	end select
end if
%>
    ...
    <table border="0" cellpadding="0" cellspacing="0">
      <tr>
		...
		<select name="search_type" id="search_type">
            <option value="users_card" <%if session("xiaofei_search_type")="users_card" then%>selected="selected"<%end if%>>会员卡号</option>
            <option value="users_name1" <%if session("xiaofei_search_type")="users_name1" then%>selected="selected"<%end if%>>会员姓名</option>
            <option value="name1" <%if session("xiaofei_search_type")="name1" then%>selected="selected"<%end if%>>商品名称</option>
			<option value="info" <%if session("xiaofei_search_type")="info" then%>selected="selected"<%end if%>>备注</option>
        </select>
	    </td>
        <td>&nbsp;&nbsp;<input name="search_key" type="text" id="search_key" value="<%=session("xiaofei_search_key")%>" onKeyPress="maskEdit(/^[\w]*$/)" /></td>
        ...
        <td align="right">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" name="Submit2" onClick="location.href='xiaofeiAM.asp?cz=Add'" value=" 添加记录 " /></td>
     </tr>
    </table>
  </form>
</center>
...
<center>
<%
sql="select * from xiaofei where id>0 "&sqladd&" order by time1 desc"
...
%>
  ...
  <%
	...
	currentpage=Clng(session("xiaofei_page"))
...
<%Do while not rs.Eof%>
        <tr onMouseOver="bgColor='#D2F5FF';" onMouseOut="bgColor='#FFFFFF';">
          ...
          <td align="center">
			...
			<a href="xiaofeiAM.asp?id=<%=rs("id")%>&amp;cz=Modify">修改</a>
		  </td>
        </tr>
<%
i=i+1
